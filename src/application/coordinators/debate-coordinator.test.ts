...
import {
  asOwnBrief,
  type AbsorbHearingAnswerInput,
  type AgentTurnInput,
  type AgentTurnResult,
  type DebateAgent,
  type HearingAnswerReview,
  type ReviewHearingAnswerInput,
  type SuggestAppealInput,
...
test("対話オーケストレータが対話から判定完了まで進める", async () => {
  ...
  session.agentMemoryA.privateBrief = asOwnBrief("A", "Aの背景");
  session.agentMemoryB.privateBrief = asOwnBrief("B", "Bの背景");
  session.agentMemoryA.publicGoal = "Aのゴール";
  session.agentMemoryB.publicGoal = "Bのゴール";
...
test("ヒアリングが入っても同じ側の手番を消費しない", async () => {
  ...
  // 依頼人へのヒアリングDMには「質問」と「確認したい理由」が含まれる
...
// 浅い回答は追撃され、具体的な回答が返ったら対話再開する。
// 最初の回答「よくわからない」→ 追撃質問 → 2回目「3月15日」→ 対話再開。
test("浅いヒアリング回答は追撃されて、具体的な回答で対話が再開する", async () => {
  const repository = new InMemorySessionRepository();
  const session = new Session({
    id: "followup-session",
...
// 追撃は maxHearingFollowups で打ち切られる。
// 回答が常に浅くても、上限を超える追撃はしない（ユーザー疲弊防止）。
test("追撃は maxHearingFollowups で打ち切られる", async () => {
  const repository = new InMemorySessionRepository();
  const session = new Session({
    id: "followup-cap",
    guildId: "guild-cap",
...
test("A代理とB代理は互いの本音を知らないまま対話する", async () => {
  ...
  session.getParticipant("A").phase = "ready";
  session.getParticipant("B").phase = "ready";
  session.agentMemoryA.privateBrief = asOwnBrief("A", A_SECRET);
  session.agentMemoryB.privateBrief = asOwnBrief("B", B_SECRET);
  session.agentMemoryA.publicGoal = "Aのゴール:GOAL_A_TOKEN";
  session.agentMemoryB.publicGoal = "Bのゴール:GOAL_B_TOKEN";
  await repository.save(session);
...
test("異議待機リスナーは案内メッセージより先に登録される", async () => {
  ...
  class OrderedMessageGateway implements MessageGateway {
    async sendDmToA(_message: string): Promise<void> {
      events.push(`dm:A`);
    }
    async sendDmToB(_message: string): Promise<void> {
      events.push(`dm:B`);
    }
    async sendTalkMessage(
      message: string,
...
// 最終審（または上告放棄）で敗者が確定したら、
// 敗者だけに振り返りメッセージを DM で送る。
// 勝者には送らず、引き分け時はどちらにも送らない。
test("敗者が確定したらConsolationDMが敗者にだけ届く", async () => {
  const repository = new InMemorySessionRepository();
  const session = new Session({
  ...
  session.getParticipant("B").phase = "ready";
  session.agentMemoryA.privateBrief = asOwnBrief("A", "Aの背景");
  session.agentMemoryB.privateBrief = asOwnBrief("B", "Bの本音:LOSER_CONTEXT_TOKEN");
  await repository.save(session);
...
// 引き分け時は誰が「敗者」か決まっていないので
// Consolation DM は送らない。
test("引き分けでは誰にもConsolationDMを送らない", async () => {
  const repository = new InMemorySessionRepository();
  const session = new Session({
...
// HEARING 上限到達時のフォールバック発言。
// maxHearingsPerSide=0 だと hearing 自体が許されないので、
// agent が hearing を返してもフォールバックメッセージで appendTurn して進む。
test("hearing 上限到達時はフォールバック発言で対話が進む", async () => {
  const repository = new InMemorySessionRepository();
  const session = new Session({
    id: "hearing-cap-zero",
    guildId: "guild-cap-zero",
    policy: new SessionPolicy({
      maxTurns: 2,
      maxAppeals: 0,
      maxHearingsPerSide: 0, // hearing 不可
    }),
  });
  session.phase = "ready";
  session.getParticipant("A").phase = "ready";
  ...
  // 開幕で B が hearing を返してくるが、上限0で却下される想定。
  class HearingOnFirstReplyAgent<
    Side extends ParticipantSide
  > extends FakeParticipantAgent<Side> {
    override async runTurn(
      input: AgentTurnInput<Side>
  ...
  const coordinator = new DebateCoordinator(
    ...
    {
      A: new FakeParticipantAgent("A"),
      B: new HearingOnFirstReplyAgent("B"),
    },
    new FakeDebateLlmGateway(),
    new FakeDebateLlmGateway(),
    messageGateway,
  ...
  const saved = await repository.findById(session.id);
  assert.ok(saved);
  // hearing が却下されたのでフォールバック発言で B のターンが消費される。
  assert.deepEqual(
    saved.getCurrentRound().turns.map((turn) => turn.speakerSide),
    ["A", "B"]
  );
  // フォールバック文言が #talk に出ている。
  assert.match(
    messageGateway.talks.map((t) => t.message).join("\n"),
  ...
  // hearing DM は送られない。
  assert.ok(
    !messageGateway.dms.some((d) => /対話中に確認したいことが出た/.test(d.message)),
    "上限0の時は hearing DM を一切送らない"
  );
});

// Consolation LLM がエラーを投げても終了フロー全体は続く。
// 敗者へのDMは送られないが、#talk の終了告知は出る。
test("Consolation LLM エラーは握りつぶされて終了フローは継続する", async () => {
  const repository = new InMemorySessionRepository();
  const session = new Session({
    id: "consolation-error",
    guildId: "guild-cons-err",
    policy: new SessionPolicy({ maxTurns: 2, maxAppeals: 0 }),
  });
  session.phase = "ready";
  session.getParticipant("A").phase = "ready";
  ...
  class ErroringConsolationGateway extends FakeDebateLlmGateway {
    override async generateConsolation(): Promise<string> {
  ...
  const coordinator = new DebateCoordinator(
    ...
    new ErroringConsolationGateway(),
    new ErroringConsolationGateway(),
    messageGateway,
    new FakeParticipantResponseGateway(),
    0
  ...
  // Consolation DM は飛ばない
  assert.ok(
    !messageGateway.dms.some((d) => /最後にひとこと/.test(d.message)),
    "LLM エラー時は Consolation DM を送らない"
  );
  // 終了アナウンスは出る
  assert.match(
    messageGateway.talks.map((t) => t.message).join("\n"),
    /もう1回やるならBotに「リセット」/,
    "終了アナウンスはエラーに左右されず必ず出る"
  );
...
// Consolation LLM が空文字列を返したら DM 送信そのものをスキップ。
// 「💬 お疲れ。\n\n」だけの空 DM が飛ばないことを保証する。
test("Consolation の空文字列レスポンスは DM 送信をスキップする", async () => {
  const repository = new InMemorySessionRepository();
  const session = new Session({
    id: "consolation-empty",
    guildId: "guild-cons-empty",
    policy: new SessionPolicy({ maxTurns: 2, maxAppeals: 0 }),
  });
  session.phase = "ready";
  session.getParticipant("A").phase = "ready";
  ...
  class EmptyConsolationGateway extends FakeDebateLlmGateway {
    override async generateConsolation(): Promise<string> {
      return "   "; // 空白のみ → 空扱い
    }
  }
