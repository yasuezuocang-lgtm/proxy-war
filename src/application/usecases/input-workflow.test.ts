...
import { APP_CONFIG_DEFAULTS } from "../../config.js";
import type {
  AppendBriefInput,
  BriefInput,
  ConsolationInput,
...
class FakeLlmGateway implements LlmGateway {
  ...
  async extractBrief(input: BriefInput): Promise<StructuredBrief> {
    this.extractInputs.push(input.rawInputs);
...
class StaleExtractLlmGateway extends FakeLlmGateway {
  async extractBrief(input: BriefInput): Promise<StructuredBrief> {
    this.extractInputs.push(input.rawInputs);
    return {
      structuredContext:
...
test("確認フェーズではAI返答の引用を除外して本人の修正だけ反映する", async () => {
  ...
  assert.match(correction.reply, /相手は動物園じゃなくて行政。/);
  assert.deepEqual(llm.appendInputs, []);
  assert.deepEqual(llm.extractInputs[llm.extractInputs.length - 1], [
    "ビーバーと共同生活したい。本気で行政に提案したい。生活環境も整える予定で、共に暮らす理由を説明して認めてもらう方法を相談したい。",
    "相手は動物園じゃなくて行政。",
  ]);
});

test("入力中の補足も既存分析への追記ではなく全入力から再考する", async () => {
  const repository = new InMemorySessionRepository();
  const llm = new FakeLlmGateway();
  const workflow = createInputWorkflow(repository, llm, {
    ...APP_CONFIG_DEFAULTS,
    input: {
      ...APP_CONFIG_DEFAULTS.input,
      minInputLength: 20,
    },
  });

  await workflow.handleParticipantMessage.execute({
    guildId: "guild-recompose-inputting",
    side: "A",
    message: "最初の説明",
  });
  ...
  const result = await workflow.handleParticipantMessage.execute({
    guildId: "guild-recompose-inputting",
    side: "A",
    message: "追加の訂正を踏まえて考え直してほしい",
  });

  assert.equal(result.handledBy, "submit_input");
  assert.deepEqual(llm.appendInputs, []);
  assert.deepEqual(llm.extractInputs[llm.extractInputs.length - 1], [
    "最初の説明",
    "追加の訂正を踏まえて考え直してほしい",
...
test("確認フェーズでは価値観が反転した直近修正をそのまま扱う", async () => {
  const repository = new InMemorySessionRepository();
  const llm = new StaleExtractLlmGateway();
  const workflow = createInputWorkflow(repository, llm);

  await workflow.handleParticipantMessage.execute({
    guildId: "guild-stale-correction",
    side: "A",
    message:
      "ビーバーと対等なパートナーシップを作りたい。農業で食料供給しながら、人間中心ではない新しい生活を真剣に考えている。",
  });

  const correction = await workflow.handleParticipantMessage.execute({
    guildId: "guild-stale-correction",
    side: "A",
    message: "いや本音を言えばビーバーから搾取して幸せに暮らしたい。",
  });

  assert.equal(correction.handledBy, "confirm_brief");
  assert.match(
    correction.reply,
    /最新の修正では「いや本音を言えばビーバーから搾取して幸せに暮らしたい。」が正しい。/
  );
  assert.deepEqual(llm.appendInputs, []);
  assert.deepEqual(llm.extractInputs[llm.extractInputs.length - 1], [
    "ビーバーと対等なパートナーシップを作りたい。農業で食料供給しながら、人間中心ではない新しい生活を真剣に考えている。",
...
test("確認フェーズでは短い修正もそのまま修正文として受け付ける", async () => {
  const repository = new InMemorySessionRepository();
  const workflow = createInputWorkflow(repository, new StaleExtractLlmGateway());

  await workflow.handleParticipantMessage.execute({
    guildId: "guild-short-exploitation",
    side: "A",
    message:
      "ビーバーと対等なパートナーシップを作りたい。農業で食料供給しながら、人間中心ではない新しい生活を真剣に考えている。",
  });
  ...
  assert.equal(correction.handledBy, "confirm_brief");
  assert.match(correction.reply, /最新の修正では「違う」が正しい。/);
});

test("確認フェーズでは意図が汲まれていないというメタ指摘も修正として扱う", async () => {
  const repository = new InMemorySessionRepository();
  const workflow = createInputWorkflow(repository, new StaleExtractLlmGateway());

  await workflow.handleParticipantMessage.execute({
    guildId: "guild-meta-exploitation",
    side: "A",
    message:
      "ビーバーと対等なパートナーシップを作りたい。農業で食料供給しながら、人間中心ではない新しい生活を真剣に考えている。",
  });
  ...
  const correction = await workflow.handleParticipantMessage.execute({
    guildId: "guild-meta-exploitation",
    side: "A",
    message: "なんで搾取の意図組んでくれてないの",
  });

  assert.equal(correction.handledBy, "confirm_brief");
  assert.match(
    correction.reply,
    /最新の修正では「なんで搾取の意図組んでくれてないの」が正しい。/
  );
});
...
test("確認フェーズでは会話ログだけの入力を修正文として採用しない", async () => {
  ...
  assert.deepEqual(llm.appendInputs, []);
  assert.equal(llm.extractInputs.length, 1);
});
